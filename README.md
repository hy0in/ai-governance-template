...
**The question that matters: will this kind of judgment need to be made repeatedly — possibly by someone other than you, possibly with the outcome acted on automatically?**

| Situation | Recommendation |
|---|---|
| A one-off judgment you need right now | Skip this template. Just ask Claude directly. |
| A judgment type that repeats, or gets split across multiple people (expense approval, refund approval, content-report review, support-ticket triage, contract-clause review, etc.) | Use this template — but you don't need golden sets and regression on day one. Start with a single rubric file, and only backfill precedents, golden cases, and regression once escalations actually start piling up. |
...
Everything this template adds in weight — rubrics, hooks, golden sets, regression, the `--no-ff` branch convention — exists to answer one requirement: when a verdict is disputed or drifts, you need to be able to trace *why* in code. A one-off judgment never generates that requirement, so it never needs the machinery.
