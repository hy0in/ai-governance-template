...
| 같은 유형의 판단을 반복적으로, 혹은 여러 사람이 나눠서 내려야 하는 일 (경비 심사, 환불 승인, 콘텐츠 신고 검토, 지원 티켓 우선순위, 계약 조항 검토 등) | 이 템플릿을 쓴다. 다만 처음부터 골든 셋·회귀까지 다 갖출 필요는 없다. 루브릭 한 장으로 시작하고, 에스컬레이션이 실제로 쌓이면 그때 판례·골든 셋·회귀를 채워 넣어도 된다 |
| 판단 결과가 자동으로 실행되거나 (자동 환불 처리 등), 여러 사람이 서로 다른 시점에 같은 유형을 판단해야 하는 일 | 이 템플릿이 정확히 겨냥하는 상황이다. 재현성·근거·감사 기록이 없으면 결과를 신뢰할 수 없다 |
...
**규범(normative/)과 지식(knowledge/)은 분리되어 있다.** 스키마·루브릭·증거 정책은 PR과 회귀 테스트 없이 수정할 수 없는 규범 자산이고, 판례·설계 결정·가설은 사람이 자유롭게 편집하는 지식 자산이다. analyst류 에이전트는 `knowledge/`를 아예 읽지 않는다. 판단이 과거 판례에 오염되지 않게 하기 위해서다.
...
이 템플릿에는 실제로 갈렸던 판단과 그 해소 과정이 예제로 남아 있다. [판례 case-001](knowledge/precedents/case-001.md)은 루브릭 문구의 모호성 때문에 같은 입력에서 결론이 갈린 사례이고, [판례 case-002](knowledge/precedents/case-002.md)는 규범 공백(루브릭 소급 적용, 판례 부재, 독립성 할인 기준) 때문에 명백한 케이스가 5회 연속 에스컬레이션된 사례다. 둘 다 사람의 판정이 규범 개정으로 이어진 전체 경로가 링크로 연결되어 있다: 매니페스트, 에스컬레이션 문서, 최종 판례.
...
`.github/workflows/`는 `normative/`·`.claude/` 변경 PR에서 훅 문법 검사와 골든 셋 `--dry` 확인만 수행한다. **실제 회귀(`run_regression.py --repeat 5`)는 CI에서 돌리지 않는다.** `claude` CLI 인증과 실행 시간이 필요해서, 항상 로컬에서 브랜치 병합 전에 사람이 실행한다.
